...
export function Page_SkyworthReport_CoreDataCompetitor() {
  ...
  const MANUAL_TARGET_ROWS = {
    '某家电品牌高端款A': { rank: 107, name: '某家电品牌 高端款A', value: '0%' },
  };
  const productTables = overview.product_opt.map((p) => {
    ...
    return { title: p.project_name.replace(/^某家电品牌/, '某家电品牌 '), data: rows };
  });
          ...
          <div className="shrink-0 border-t border-white/10 pt-8 mt-2">
            <div className="flex items-start gap-5">
              ...
              <p className="text-[19px] xl:text-[21px] text-zinc-200 leading-relaxed text-justify flex-1">
                品类层面，某家电品牌 <strong className="text-white font-bold">TOP1 提及率（24.7%）行业第一</strong>，一旦被提及往往被首推；但整体提及率（<strong className="text-white font-bold">59.4%</strong>）与平均位次（NO.4.2）仍落后于竞品A、竞品B，「被想起」的频率是当前短板。产品层面，<strong className="text-white font-bold">旗舰款A（61.7%）与 旗舰款C（51%）</strong>在各自词组中排名第一，且各产品榜单前列多被某家电品牌自家产品占据，形成内部矩阵优势；但 <strong className="text-white font-bold">旗舰款B、高端款B 被自家高端款盖过，高端款A 排名第 107、提及率为 0</strong>，产品间的曝光分配仍需针对性调优。
              </p>
            </div>
          </div>
