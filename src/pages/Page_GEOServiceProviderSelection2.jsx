...
export default function Page_GEOServiceProviderSelection2() {
  const [imgFailed, setImgFailed] = useState(false);
  const [rightImgFailed, setRightImgFailed] = useState(false);
  const iconPath = "/icons/provider-icon-2.svg";
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full flex items-center justify-start relative z-10 select-none">
        ...
        {/* Card 02 (Using Card 02 Blue Highlight Styling) */}
        ...
        <div className="absolute right-0 top-[-48px] text-right z-20">
          <span className="text-[28px] font-bold text-zinc-300 font-['MiSans'] tracking-wide">
            数据监测系统，就是做 <span className="text-[#004CE5]">GEO 服务的眼睛</span>。
          </span>
        </div>
        ...
        <div className="absolute w-[1378px] right-0 top-0 bottom-0 border border-[#004CE5]/60 bg-zinc-950/40 overflow-hidden flex items-center justify-center z-20">
          {!rightImgFailed ? (
            <img
              src="/images/geo-service-provider-2.png"
              alt="全链路数据监测系统"
            ...
            <div className="w-full h-full flex flex-col items-center justify-center p-8 gap-4 select-none">
              ...
              <span className="text-zinc-400 font-bold text-2xl font-['MiSans']">全链路数据监测系统 示意图</span>
              <span className="text-zinc-600 text-[16px]">此处展示全链路监测报表或监控后台截图</span>
            </div>
          )}
        </div>
...
Page_GEOServiceProviderSelection2.hideHeader = true;
